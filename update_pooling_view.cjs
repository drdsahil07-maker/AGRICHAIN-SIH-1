const fs = require('fs');
let content = fs.readFileSync('frontend/src/components/DynamicPoolingView.tsx', 'utf-8');
...
export const DynamicPoolingView: React.FC<DynamicPoolingViewProps> = ({ onInspectPool }) => {
  ...
  const handleConfirmPool = async (suggestion: any) => {
    try {
      ...
      setSuggestions(suggestions.filter(s => s.poolCode !== suggestion.poolCode));
      fetchData();
    } catch (e: any) {
...
fs.writeFileSync('frontend/src/components/DynamicPoolingView.tsx', replacement);
