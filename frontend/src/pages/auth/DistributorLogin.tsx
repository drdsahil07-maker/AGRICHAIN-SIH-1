...
export const DistributorLogin: React.FC = () => {
  ...
  const handleFillDemo = async (autoSubmit: boolean = false) => {
    setIdentifier('distributor@agrichain.com');
    setPassword('Password123!');
    setError(null);
    if (autoSubmit) {
      setLoading(true);
      try {
        const result = await login('distributor@agrichain.com', 'Password123!', 'distributor');
        setLoading(false);
        if (result.success) {
          navigate('/distributor/dashboard');
        } else {
          setError(result.error || 'Failed to sign in as Distributor.');
        }
      } catch (err: any) {
        setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await login(identifier, password, 'distributor');
      setLoading(false);

      if (result.success) {
        navigate('/distributor/dashboard');
      } else {
        setError(result.error || 'Failed to sign in as Distributor. Please check credentials.');
      }
    } catch (err: any) {
      setLoading(false);
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <div className="text-center space-y-2 mb-6">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Source produce directly from verified farmer clusters with escrow protection
          </p>
        </div>
        ...
        <div className="bg-white py-8 px-6 sm:px-8 shadow-md border border-slate-200 rounded-3xl space-y-5">
          ...
          <div className="p-3.5 bg-amber-50/80 border border-amber-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div>
              <div className="font-bold text-amber-950 flex items-center gap-1.5">
                ...
                <span>Distributor Demo Credentials</span>
              </div>
              <div className="text-amber-800 font-mono text-[11px] mt-0.5 select-all">
                Email: <strong>distributor@agrichain.com</strong> &bull; Pass: <strong>Password123!</strong>
              </div>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                ...
                className="px-2.5 py-1 bg-white hover:bg-amber-100 text-amber-900 font-bold text-[11px] rounded-lg border border-amber-300 transition-colors cursor-pointer"
              >
                Auto-Fill
              </button>
              <button
                ...
                className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-[11px] rounded-lg shadow-xs transition-colors cursor-pointer"
              >
                1-Click Login
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. procurement@distributor.com"
            ...
            <div className="space-y-2 pt-2">
              ...
              >
                ...
                <span>{loading ? 'Signing in...' : 'Login to Distributor Portal'}</span>
              </button>
              ...
              >
                ...
                <span>Create New Distributor Account</span>
              </button>
            </div>
          </form>
          ...
          <div className="pt-2 text-center border-t border-slate-100 flex justify-between items-center text-xs">
            <Link 
              to="/choose-role" 
              className="font-semibold text-slate-500 hover:text-amber-700 transition-colors"
            >
              ← Change role
            </Link>
            <Link
              to="/forgot-password"
              className="font-semibold text-amber-700 hover:underline transition-colors"
            >
              Forgot Password?
            </Link>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-7 max-w-md w-full shadow-2xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Enter your registered business email to receive a secure password reset link.
            </p>
            ...
            <form onSubmit={handlePasswordReset} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium focus:bg-white focus:ring-2 focus:ring-amber-500 focus:outline-none"
                />
              </div>
